...
int main(void)
{
  ...
  sprintf(msg, "MPU6050 initialized successfully!\r\n");
  ...
  while (1)
  {
	  ...
	  float accel_roll  = atan2f(ay_g, az_g) * 180.0f / 3.14159f;
	  float accel_pitch = atan2f(-ax_g, sqrtf(ay_g * ay_g + az_g * az_g)) * 180.0f / 3.14159f;

	  // Update Kalman filter
	  float roll_angle = kalman_get_angle(&roll_kalman, accel_roll, gx_dps, dt);
	  float pitch_angle = kalman_get_angle(&pitch_kalman, accel_pitch, gy_dps, dt);

	  sprintf(msg, "ROLL: %.2f | PITCH: %.2f\r\n", roll_angle, pitch_angle);
	  HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), 100);
