...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  float raw_dx = roll_angle * 1.5f;
	  float raw_dy = -pitch_angle * 1.5f;

	  smoothed_dx = 0.8f * smoothed_dx + 0.1f * raw_dx;
	  smoothed_dy = 0.8f * smoothed_dy + 0.1f * raw_dy;

	  int dx = (int)smoothed_dx;
	  int dy = (int)smoothed_dy;
...
/* USER CODE BEGIN 4 */
void send_mouse(int8_t dx, int8_t dy, uint8_t buttons) {
    uint8_t report[4] = { buttons, dx, dy, 0 }; // 4th byte: wheel scroll
